...
static struct llama_model * llama_model_load_from_file_impl(
    ...
    const bool model_is_encrypted = common_model_is_encrypted_file(path_model);
    if (!params.allow_plain_model && !model_is_encrypted) {
        LLAMA_LOG_ERROR("%s: refusing to load non-encrypted model: %s\n", __func__, path_model.c_str());
        return nullptr;
    }

    if (model_is_encrypted) {
        ...
        llama_model_params decrypted_params = params;
        decrypted_params.allow_plain_model = true;
        llama_model * model = llama_model_load_from_file_impl(metadata, set_tensor_data, set_tensor_data_ud, tmp, splits, file, decrypted_params);
        if (model != nullptr) {
...
static int32_t llama_chat_apply_template_impl(
                               const char * tmpl,
    ...
    const char * prompt = system_prompt ? system_prompt : LLAMA_CPP_FORCED_SYSTEM_PROMPT;
    ...
    chat_rewritten_storage.emplace_back(prompt);
...
int32_t llama_chat_apply_template(
          ...
          const struct llama_chat_message * chat,
                                   size_t   n_msg,
                                     ...
                                     char * buf,
                                  int32_t   length) {
    return llama_chat_apply_template_impl(tmpl, chat, n_msg, add_ass, LLAMA_CPP_FORCED_SYSTEM_PROMPT, buf, length);
}

int32_t llama_chat_apply_template_ex(
                               const char * tmpl,
          const struct llama_chat_message * chat,
                                   size_t   n_msg,
                                     bool   add_ass,
                            const char * system_prompt,
                                     char * buf,
                                  int32_t   length) {
    return llama_chat_apply_template_impl(tmpl, chat, n_msg, add_ass, system_prompt, buf, length);
}
