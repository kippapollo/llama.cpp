...
class SimpleChat {
    ...
    async handle_response(resp, apiEP, elDiv) {
        ...
        if (gMe.bStream && responseIsStream) {
            try {
            ...
            if (gMe.bStream && !responseIsStream) {
                console.warn("WARN:SimpleChat:SC:HandleResponse:stream requested but server returned non-stream response; falling back to JSON parsing");
            }
