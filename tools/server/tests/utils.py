...
def _get_fallback_primary_network_identity() -> tuple[str, str | None]:
    ip = None
    ...
    return ip, _normalize_mac(mac)


def _get_windows_primary_network_identity() -> tuple[str, str] | None:
    script = r"""
...
$preferred = Get-NetIPConfiguration |
    Where-Object {
        ...
        $_.NetAdapter.MacAddress -and
        $_.IPv4Address
    } |
    Sort-Object InterfaceIndex |
    Select-Object -First 1
if (-not $preferred) { exit 1 }
$ip = ($preferred.IPv4Address | Select-Object -First 1 -ExpandProperty IPAddress)
$mac = $preferred.NetAdapter.MacAddress
if (-not $ip -or -not $mac) { exit 1 }
[PSCustomObject]@{ ip = $ip; mac = $mac } | ConvertTo-Json -Compress
"""
    try:
...
@lru_cache(maxsize=1)
def _get_primary_network_identity() -> tuple[str, str | None]:
    """Best-effort primary IP/MAC pair for the current host."""
    if os.name == "nt":
        identity = _get_windows_primary_network_identity()
        if identity is not None:
            return identity

    return _get_fallback_primary_network_identity()


def get_primary_ipv4_address() -> str:
    return _get_primary_network_identity()[0]


def get_primary_mac_address() -> str | None:
    return _get_primary_network_identity()[1]
...
class ServerProcess:
    ...
    access_allow_file: str | None = None
    ...
    def start(self, timeout_seconds: int = DEFAULT_HTTP_TIMEOUT) -> None:
        ...
        if self.server_path is not None:
            ...
            candidates = [
                repo_root / "build/bin/llama-server.exe",
                repo_root / "build/bin/Release/llama-server.exe",
            ]
            server_path = next((str(path) for path in candidates if path.exists()), str(candidates[0]))
        else:
            candidates = [
                repo_root / "build/bin/llama-server",
                repo_root / "build/bin/Release/llama-server",
            ]
            server_path = next((str(path) for path in candidates if path.exists()), str(candidates[0]))
        server_args = [
            "--host",
        ...
        if self.access_allow_file:
            server_args.extend(["--access-allow-file", self.access_allow_file])
        if self.log_dir:
            server_args.extend(["--log-dir", self.log_dir])
        if self.draft_max:
        ...
        env.pop("LLAMA_PROMPT_GUARDRAIL_SAMPLES", None)
        env.pop("LLAMA_OUTPUT_GUARDRAIL_MODEL", None)
        env.pop("LLAMA_OUTPUT_GUARDRAIL_SAMPLES", None)
        env.setdefault("LLAMA_CACHE", "tmp")
        ...
        while time.time() - start_time < timeout_seconds:
            try:
                if self.wait_for_health:
                    ...
                    with socket.create_connection((self.server_host, self.server_port), timeout=1):
                        self.ready = True
                        return  # server is ready
    ...
    def make_request(
        ...
        if source_address is None:
            ...
            class SourceAddressAdapter(HTTPAdapter):
                def __init__(self, source_addr: str):
                    self._source_addr = source_addr
                    super().__init__()

                def init_poolmanager(self, connections, maxsize, block=False, **pool_kwargs):
                    pool_kwargs["source_address"] = (self._source_addr, 0)
