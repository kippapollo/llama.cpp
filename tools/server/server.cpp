...
static bool parse_server_extra_args(
        int argc,
        char ** argv,
        server_http_extra_params * extra_params,
        std::vector<std::string> * filtered_storage,
        std::vector<char *> * filtered_argv,
        std::string * error) {
    if (extra_params == nullptr || filtered_storage == nullptr || filtered_argv == nullptr) {
        if (error != nullptr) {
            *error = "internal argument parsing error";
        }
        return false;
    }
    ...
    auto append_arg = [&](const std::string & arg) {
        filtered_storage->push_back(arg);
        filtered_argv->push_back(filtered_storage->back().data());
    };

    auto consume_option = [&](int & i, const char * option, std::string * value) -> bool {
    ...
    append_arg(argv[0] != nullptr ? argv[0] : "");
    for (int i = 1; i < argc; ++i) {
        std::string value;
        if (consume_option(i, "--access-allow-file", &value)) {
            extra_params->access_allow_file = std::move(value);
            continue;
        }
...
int main(int argc, char ** argv) {
    ...
    std::vector<std::string> filtered_storage;
    std::vector<char *> filtered_argv;
    std::string extra_arg_error;
    if (!parse_server_extra_args(argc, argv, &extra_params, &filtered_storage, &filtered_argv, &extra_arg_error)) {
        LOG_ERR("%s\n", extra_arg_error.c_str());
        return 1;
    }

    if (!common_params_parse(static_cast<int>(filtered_argv.size()), filtered_argv.data(), params, LLAMA_EXAMPLE_SERVER)) {
        return 1;
    }
    ...
    if (!ctx_http.init(params, extra_params)) {
