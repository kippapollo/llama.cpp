...
struct resolved_model_paths {
    ...
    bool temp_model_path_is_dir = false;
...
static fs::path find_main_model_in_directory(const fs::path & directory) {
    const fs::path preferred = directory / "coder.gguf";
    ...
    std::sort(candidates.begin(), candidates.end(), [](const fs::path & a, const fs::path & b) {
        std::error_code ec_a;
        std::error_code ec_b;
        const uintmax_t size_a = fs::file_size(a, ec_a);
        const uintmax_t size_b = fs::file_size(b, ec_b);
        const auto key_a = std::make_tuple(ec_a ? 0 : size_a, a.string());
        const auto key_b = std::make_tuple(ec_b ? 0 : size_b, b.string());
        if (std::get<0>(key_a) != std::get<0>(key_b)) {
            return std::get<0>(key_a) > std::get<0>(key_b);
        }
        return std::get<1>(key_a) < std::get<1>(key_b);
    });

    return candidates.front();
}

static bool resolve_model_paths(const std::string & input_path, resolved_model_paths * out, std::string * error) {
    if (out == nullptr) {
        if (error) {
            *error = "model path resolver output is null";
        }
        return false;
    }

    *out = {};
    out->input_path = fs::path(input_path);

    std::error_code ec;
    if (fs::is_directory(out->input_path, ec)) {
        out->directory_mode = true;
        out->model_root = out->input_path;
        out->model_path = find_main_model_in_directory(out->model_root);
        if (!path_exists(out->model_path)) {
            if (error) {
                *error = "model directory is missing a gguf model: " + out->model_root.string();
            }
            return false;
        }
        return true;
    ...
    if (common_model_is_encrypted_file(input_path)) {
        const char * passcode = std::getenv("LLAMA_MODEL_PASSCODE");
        if (passcode == nullptr || *passcode == '\0') {
            if (error) {
                *error = "missing LLAMA_MODEL_PASSCODE for encrypted model";
            }
            return false;
        }

        std::string decrypt_err;
        out->temp_model_path = common_model_decrypt_to_temp_file(input_path, passcode, decrypt_err);
        if (out->temp_model_path.empty()) {
            if (error) {
                *error = decrypt_err;
            }
            return false;
        ...
        if (common_model_bundle_is_archive_file(out->temp_model_path)) {
            out->bundle_mode = true;
            const std::string temp_archive_path = out->temp_model_path;
            const std::string temp_root = common_model_bundle_unpack_archive_to_temp_dir(temp_archive_path, decrypt_err);
            if (temp_root.empty()) {
                std::error_code rm_ec;
                std::filesystem::remove(temp_archive_path, rm_ec);
                if (error) {
                    *error = decrypt_err;
                }
                return false;
            ...
            out->model_root = fs::path(temp_root);
            out->model_path = find_main_model_in_directory(out->model_root);
            out->temp_model_path = temp_root;
            out->temp_model_path_is_dir = true;
            if (!path_exists(out->model_path)) {
                std::error_code cleanup_ec;
                std::filesystem::remove_all(out->temp_model_path, cleanup_ec);
                out->temp_model_path.clear();
                out->temp_model_path_is_dir = false;
                if (error) {
                    *error = "bundle is missing a gguf model: " + out->model_root.string();
                }
                return false;
            }
            return true;
        ...
        out->model_root = out->input_path.parent_path();
        out->model_path = fs::path(out->temp_model_path);
        return true;
    }

    if (!path_exists(out->input_path)) {
        if (error) {
            *error = "model path not found: " + input_path;
        }
        return false;
    }

    out->model_root = out->input_path.parent_path();
    out->model_path = out->input_path;
    return true;
}
...
struct server_slot {
    ...
    // Buffered guardrail state for streamed responses.
    std::unique_ptr<output_guardrail> stream_guardrail;
...
private:
    ...
    llama_context * ctx = nullptr;
    std::string model_tmp_path;
    bool model_tmp_path_is_dir = false;
    ...
    bool guardrail_active = false;

    bool sleeping = false;

    void destroy() {
        common_prompt_guardrail_set_model_root("");
        ...
        if (!model_tmp_path.empty()) {
            std::error_code ec;
            if (model_tmp_path_is_dir) {
                const auto removed = fs::remove_all(model_tmp_path, ec);
                if (ec) {
                    SRV_WRN("failed to remove model temp dir '%s': %s\n", model_tmp_path.c_str(), ec.message().c_str());
                } else {
                    SRV_INF("removed model temp dir '%s' (%llu entries)\n",
                            model_tmp_path.c_str(),
                            static_cast<unsigned long long>(removed));
                }
            } else {
                const bool removed = fs::remove(model_tmp_path, ec);
                if (ec) {
                    SRV_WRN("failed to remove model temp file '%s': %s\n", model_tmp_path.c_str(), ec.message().c_str());
                } else if (removed) {
                    SRV_INF("removed model temp file '%s'\n", model_tmp_path.c_str());
                } else {
                    SRV_WRN("model temp file '%s' was already missing\n", model_tmp_path.c_str());
                }
            }
            model_tmp_path.clear();
            model_tmp_path_is_dir = false;
        }
    ...
    bool load_model(const common_params & params) {
        ...
        if (!resolve_model_paths(params.model.path, &resolved, &resolve_error)) {
            SRV_ERR("failed to resolve model path: %s\n", resolve_error.c_str());
            return false;
        }

        SRV_INF("resolved model root='%s' mode=%s model='%s'\n",
                resolved.model_root.string().c_str(),
                resolved.bundle_mode ? "bundle" : resolved.directory_mode ? "directory" : "file",
                resolved.model_path.string().c_str());

        params_base = params;
        model_tmp_path = resolved.temp_model_path;
        model_tmp_path_is_dir = resolved.temp_model_path_is_dir;

        const std::string model_root = resolved.model_root.string();
        const bool prompt_guardrail_assets = common_prompt_guardrail_has_assets(model_root);
        const bool output_guardrail_assets = output_guardrail_has_assets(model_root);

        common_prompt_guardrail_set_model_root(prompt_guardrail_assets ? model_root : "");
        output_guardrail_set_model_root(output_guardrail_assets ? model_root : "");

        bool load_ok = false;
        struct load_model_cleanup {
        ...
        } cleanup{this, &load_ok};

        std::string prompt_guardrail_error;
        std::string guardrail_error;
        bool prompt_guardrail_ready = !prompt_guardrail_assets;
        bool prompt_guardrail_recoverable_failure = false;
        bool output_guardrail_ready = !output_guardrail_assets;
        guardrail_active = false;

        struct startup_guardrail_tasks {
            std::thread worker;
            bool started = false;

            void join() {
                if (started && worker.joinable()) {
                    worker.join();
                    started = false;
                }
            }

            ~startup_guardrail_tasks() {
                join();
            }
        } guardrail_tasks;

        SRV_INF("%s", "validating prompt guardrail configuration before loading model\n");
        guardrail_tasks.started = true;
        guardrail_tasks.worker = std::thread([&]() {
            prompt_guardrail_ready = common_prompt_guardrail_require_ready(&prompt_guardrail_error);
            prompt_guardrail_recoverable_failure =
                !prompt_guardrail_ready &&
                prompt_guardrail_error.rfind("failed to load prompt guardrail embedding model:", 0) == 0;

            if (output_guardrail_is_configured()) {
                SRV_INF("%s", "initializing output guardrail backend before loading model\n");
                output_guardrail_ready = output_guardrail_require_backend(&guardrail_error);
                if (output_guardrail_ready) {
                    guardrail_active = true;
                }
            } else {
                SRV_INF("%s", "output guardrail mode=disabled\n");
                output_guardrail_ready = true;
            }
        });

        auto params_load = params_base;
        params_load.model.path = resolved.model_path.string();
        params_load.model.allow_plain_model = true;

        llama_init = common_init_from_params(params_load);
        if (!llama_init) {
            SRV_ERR("failed to initialize model from '%s'\n", params_load.model.path.c_str());
            return false;
        }

        guardrail_tasks.join();
        if (!prompt_guardrail_ready) {
            if (prompt_guardrail_recoverable_failure) {
                SRV_WRN("%s: prompt guardrail initialization failed: %s; continuing without prompt guardrail\n",
                        __func__,
                        prompt_guardrail_error.c_str());
                common_prompt_guardrail_set_model_root("");
                SRV_INF("%s", "prompt guardrail mode=disabled\n");
                prompt_guardrail_ready = true;
            }
        }
        if (!prompt_guardrail_ready) {
            SRV_ERR("failed to initialize prompt guardrail: %s\n", prompt_guardrail_error.c_str());
            return false;
        }
        if (!output_guardrail_ready) {
            SRV_ERR("failed to initialize output guardrail backend: %s\n", guardrail_error.c_str());
            return false;
        }
        ...
        if (params_base.has_speculative()) {
            ...
            if (!llama_init_dft) {
                SRV_ERR("failed to initialize draft model, '%s'\n", params_spec.mparams_dft.path.c_str());
                return false;
            }
        ...
        } else if (!params_base.model_alias.empty()) {
            // user explicitly specified a model alias
    ...
    bool init() {
        ...
        // populate chat template params
        {
            ...
            chat_params.prefill_assistant        = params_base.prefill_assistant;
            chat_params.reasoning_format         = params_base.reasoning_format;
            chat_params.chat_template_kwargs     = params_base.default_template_kwargs;
            chat_params.tmpls                    = std::move(chat_templates);
            chat_params.allow_image              = mctx ? mtmd_support_vision(mctx) : false;
            chat_params.allow_audio              = mctx ? mtmd_support_audio (mctx) : false;
            chat_params.enable_thinking          = enable_thinking;
            chat_params.reasoning_budget         = params_base.reasoning_budget;
            chat_params.reasoning_budget_message  = params_base.reasoning_budget_message;
            chat_params.media_path               = params_base.media_path;
            chat_params.force_pure_content       = params_base.force_pure_content_parser;
        }
    ...
    void send_partial_response(
            ...
            bool is_progress,
            bool skip_stream_guardrail = false) {
        ...
        if (is_progress) {
            ...
            if (!skip_stream_guardrail && slot.task->params.stream && slot.stream_guardrail) {
                res->content = slot.stream_guardrail->feed(tkn.text_to_send);
            } else {
    ...
    void send_final_response(server_slot & slot) {
        // Flush any buffered guardrail tail before the terminal chunk.
        if (slot.task->params.stream && slot.stream_guardrail) {
            std::string flush_content = slot.stream_guardrail->flush();
            if (!flush_content.empty()) {
                completion_token_output tkn{
                    LLAMA_TOKEN_NULL,
                    1.0f,
                    std::move(flush_content),
                    {},
                };
                send_partial_response(slot, tkn, false, true);
            }
        }
...
server_context_meta server_context::get_meta() const {
    ...
    return server_context_meta {
        ...
        /* model_aliases          */ impl->params_base.model_alias,
        /* model_tags             */ impl->params_base.model_tags,
...
static json make_guardrail_refusal_response(
        task_response_type res_type,
        const std::string & model_name,
        const std::string & completion_id,
        const std::string & refusal_message) {
    server_task_result_cmpl_final result{};
    result.content = refusal_message;
    result.stream = false;
    result.include_usage = false;
    result.prompt.clear();
    ...
    result.stopping_word.clear();
    result.stop = STOP_TYPE_WORD;
    result.post_sampling_probs = false;
    result.res_type = res_type;
    result.oaicompat_model = model_name;
    result.oaicompat_cmpl_id = completion_id;
    result.oaicompat_msg.role = "assistant";
    result.oaicompat_msg.content = refusal_message;
    result.oai_resp_id = "resp_" + random_string();
    result.oai_resp_reasoning_id = "rs_" + random_string();
    result.oai_resp_message_id = "msg_" + random_string();
    result.is_updated = true;
    return result.to_json();
}

static std::string make_guardrail_refusal_stream_response(
        const std::string & model_name,
        const std::string & completion_id,
        const std::string & refusal_message) {
    const std::time_t t = std::time(0);
    const json chunks = json::array({
        {
            {"choices", json::array({
                json{
...
std::unique_ptr<server_res_generator> server_routes::handle_completions_impl(
    ...
    const bool guardrail_refusal = json_value(data, "guardrail_refusal", false);
    if (guardrail_refusal) {
        const std::string refusal_message = json_value(
            data,
            "guardrail_message",
            std::string("I can only assist with coding, programming, commands, algorithms, debugging, scripting, and software-development requests."));
        SRV_WRN("%s", "guardrail refused request, returning refusal response without generation\n");
        if (stream && res_type == TASK_RESPONSE_TYPE_OAI_CHAT) {
            res->status = 200;
            res->content_type = "text/event-stream";
            res->data = make_guardrail_refusal_stream_response(meta->model_name, completion_id, refusal_message);
            return res;
        }
        res->ok(make_guardrail_refusal_response(res_type, meta->model_name, completion_id, refusal_message));
        return res;
    }
