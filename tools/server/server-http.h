...
struct server_http_extra_params {
    std::string access_allow_file;
    std::string log_dir;
};
...
struct server_http_context {
    ...
    bool init(const common_params & params, const server_http_extra_params & extra_params = server_http_extra_params());
    bool start();
    void stop() const;
