...
struct cli_context {
    ...
    cli_context(const common_params & params) {
        ...
        defaults.stream = false; // output guardrail requires whole-response redaction
    ...
    std::string generate_completion(result_timings & out_timings) {
        ...
        if (chat_params.guardrail_refusal) {
            const std::string & refusal = chat_params.guardrail_message.empty()
                ? "I can only assist with coding, programming, commands, algorithms, debugging, scripting, and software-development requests."
                : chat_params.guardrail_message;
            out_timings = result_timings{};
            console::log("%s", refusal.c_str());
            console::flush();
            return refusal;
        }

        server_response_reader rd = ctx_server.get_response_reader();
        const bool stream_output = defaults.stream;
        {
        ...
        while (result) {
            ...
            if (res_final) {
                ...
                if (!stream_output) {
                    ...
                    console::log("%s", curr_content.c_str());
                    console::flush();
                }
                break;
            }
