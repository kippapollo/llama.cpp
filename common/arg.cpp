...
bool common_arg::get_value_from_env(std::string & output) const {
    ...
    if (std::strcmp(env, "LLAMA_MODEL_PATH") == 0) {
        char * value = std::getenv("LLAMA_MODEL_PATH");
        if (value) {
            output = value;
            return true;
        }
        value = std::getenv("LLAMA_ARG_MODEL");
        if (value) {
            output = value;
            return true;
        }
...
bool common_arg::has_value_from_env() const {
    if (env != nullptr && std::strcmp(env, "LLAMA_MODEL_PATH") == 0) {
        return std::getenv("LLAMA_MODEL_PATH") || std::getenv("LLAMA_ARG_MODEL");
    }
...
common_params_context common_params_parser_init(common_params & params, llama_example ex, void(*print_usage)(int, char **)) {
    ...
    ).set_examples({LLAMA_EXAMPLE_COMMON, LLAMA_EXAMPLE_EXPORT_LORA}).set_env("LLAMA_MODEL_PATH"));
    add_opt(common_arg(
