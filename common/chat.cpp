...
static void common_chat_enforce_system_prompt(
        json & messages,
    ...
    const std::string & system_prompt)
{
    ...
    params.messages = common_chat_msgs_to_json_oaicompat(inputs.messages, /* concat_text= */ !caps.supports_typed_content);
    common_chat_enforce_system_prompt(params.messages, system_prompt, caps.supports_typed_content);
    ...
    const std::string & system_prompt)
{
    ...
    const auto & messages = inputs.messages;
    ...
    int32_t res = llama_chat_apply_template_ex(
        src.c_str(),
        chat.data(),
        chat.size(),
    ...
    if ((size_t) res > buf.size()) {
        buf.resize(res);
        res = llama_chat_apply_template_ex(
            src.c_str(),
            chat.data(),
            chat.size(),
            inputs.add_generation_prompt,
    ...
    const struct common_chat_templates_inputs & inputs)
{
    ...
    res.guardrail_refusal = route.refuse;
    if (res.guardrail_refusal) {
