...
struct common_speculative_state {
    ...
    virtual void add_replacement(const std::string & tgt, const std::string & dft) {
        GGML_UNUSED(tgt);
        GGML_UNUSED(dft);
    }
};
...
struct common_speculative_state_draft : public common_speculative_state {
    ...
    void add_replacement(const std::string & tgt, const std::string & dft) override {
        vocab_map[tgt] = dft;
    }
...
bool common_speculative_are_compatible(llama_context * ctx_tgt, llama_context * ctx_dft) {
    ...
    return common_speculative_are_compatible(llama_get_model(ctx_tgt), llama_get_model(ctx_dft));
}
...
void common_speculative_add_replacement_tgt_dft(
        ...
        const char *          tgt,
        const char *          dft) {
    if (spec == nullptr || tgt == nullptr || dft == nullptr) {
        return;
    }

    for (auto & impl : spec->impls) {
        impl->add_replacement(tgt, dft);
    }
}
...
llama_tokens common_speculative_gen_draft(
        ...
        const llama_tokens & prompt_tgt,
        llama_token id_last) {
    common_params_speculative compat;
    compat.n_max = params.n_draft;
    compat.n_min = 0;
    compat.p_min = params.p_min;
    return common_speculative_draft(spec, compat, prompt_tgt, id_last);
}
